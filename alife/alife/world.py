...
class World:
    ...
    def __init__(self,fname=None,init_sprites=2,graphics=1):
        ...
        agents_available2 = get_list('agents_to_use2.txt')


        # Some animate creatures
        for i in range(int(self.N_ROWS*FACTOR/4*2)):
            Creature((random_position(self)), dna = agents_available[random.choice(len(agents_available))], energy = 75, energy_limit = 200)
        for i in range(int(self.N_ROWS*FACTOR/6*2)):
            Creature((random_position(self)), dna = agents_available2[random.choice(len(agents_available2))], energy = 200, energy_limit = 450, ID = ID_PREDATOR, food_ID = ID_ANIMAL)
        ...
        while True:
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    return
                if event.type == pygame.KEYDOWN & graphics==1:
