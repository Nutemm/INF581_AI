...
class Thing(pygame.sprite.DirtySprite):
    ...
    def hit_by(self, creature):
        ...
        if self.ID == ID_PLANT and creature.ID == ID_ANIMAL:
            ...
            self.energy = self.energy - MINIMIZE*bite     #
...
class Creature(Thing):
    ...
    def hit_by(self, being):
        ...
        if self.ID < being.ID:
            ...
            if angle_of_attack(predator,prey) > pi/3.:
                ...
                predator.energy = predator.energy - speed_of_attack * BOUNCE_DAMAGE * MINIMIZE   # Ouch!
                prey.energy = prey.energy - speed_of_attack * BOUNCE_DAMAGE *MINIMIZE        # Ouch!
            else:
                bite = speed_of_attack * random.rand() * 10.
                # Attacker successful (bites off %3.2f cal). % bite
                predator.energy = predator.energy + bite*MINIMIZE
                prey.energy = prey.energy - bite *MINIMIZE
                BounceOffFrom(predator,prey)

        else:
            # The being is from the same species (TODO: add reproduction option here)
            self.energy = self.energy - norm(being.velocity) * BOUNCE_DAMAGE * MINIMIZE   # Ouch!
            being.energy = being.energy - norm(self.velocity) * BOUNCE_DAMAGE * MINIMIZE # Ouch!
            BounceOffFrom(self,being)

    def live(self, world):

        # Burn energy (for being alive)
        self.energy = self.energy - (0.001 * self.radius) * MINIMIZE
        ...
        if norm(self.velocity) < FLIGHT_SPEED:
            ...
            if terrain_centre is not None:
                ...
                self.energy = self.energy -  norm(self.velocity) * TERRAIN_DAMAGE * MINIMIZE
    ...
    def process_actions(self,action):
        ...
        self.energy = self.energy - burn(angle, speed, self.radius) * MINIMIZE
