...
nb_games = 1
nb_frames = 0 # 30 frames per second
...
while 1:
    ...
    if nb_frames%10==0 and nb_frames>=agent.memory.maxlen:
        agent.updateEvolver()
        last_losses.append(round(sum(abs((-agent.model.predict(observation)+ \
        reward+agent.gamma*agent.model.predict(p.getGameState()))[0])),3))
